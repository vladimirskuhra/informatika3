...
Node::Node(ICommand *cmd, Cursor *cur) : 
m_command(cmd), 
m_cursor(cur), 
m_parent(nullptr), 
m_isFocused(false) {}
...
Node::~Node() { 
    delete m_cursor; 
    delete m_command; 
    for (Node* s : m_subnodes) delete s; 
}

Node* Node::createLeafNode(ICommand *cmd) { 
    auto* c = new CursorUp(); 
    auto* n = new Node(cmd, c); 
    c->setNode(n); 
    return n; }
Node* Node::createSequentialNode() { 
    auto* c = new SequentialCursor(); 
    auto* n = new Node(nullptr, c); 
    c->setNode(n); 
    return n; 
}
std::string Node::toString() const { 
    std::stringstream ss; 
    if (m_isFocused) ss << "> "; 
    if (m_command) ss << m_command->toString(); 
    else ss << "Sequential"; 
    if (m_cursor) ss << " [" << m_cursor->toString() << "]"; 
    return ss.str(); 
}
void Node::addSubnode(Node *sub) { 
    if (!sub) throw std::invalid_argument("subnode is nullptr"); 
...
Node* SequentialCursor::next() { 
    m_current++; 
    if (static_cast<size_t>(m_current) < m_node->getSubnodes().size()) 
...
std::string SequentialCursor::toString() {
    if (static_cast<size_t>(m_current + 1) < m_node->getSubnodes().size()) {
        return "Child " + std::to_string(m_current + 2) + "/" +
               std::to_string(m_node->getSubnodes().size());
    }
    return "Up";
}


Interpreter::Interpreter(Node* r) : 
m_root(r), 
m_current(nullptr), 
...
void Interpreter::interpretStep(Controllable& c) {
    if (isFinished()) return;

    ICommand* cmd = m_current->getCommand();
    if (cmd) {
        if (cmd->canBeExecutedOn(c)) {
            cmd->log(std::cout);
            cmd->execute(c);
        ...
        if (m_stopOnNodeWithoutCommand) {
            std::cout << "Node without command.\n";
        }
    }
    moveCurrent();
}
...
void Interpreter::interpretAll(Controllable& c) {
    reset();
    c.reset();
    while (!isFinished()) {
        ICommand* cmd = m_current->getCommand();
